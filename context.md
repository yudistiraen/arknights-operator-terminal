...
**Catatan batch Almond/Astesia/Astgenne/Breeze/Nowell/Tequila:**
...
- Stats (hp/atk/def/res/block/cost/aspd/rdp) untuk batch ini diverifikasi silang langsung dari `character_table.json` (`phases[].attributesKeyFrames` level E2 max + `favorKeyFrames` buat bonus trust), bukan cuma dari angka wiki — ternyata cocok persis di semua kasus baru (Almond, Astesia, Astgenne, Breeze, Nowell, Tequila, dan cross-check Ulpianus dari batch lama). ⚠️ Beberapa operator LAMA di file ini (Cantabile, Insider) staknya **tidak** cocok dengan gamedata terkini (selisih kecil, kemungkinan besar karena rebalance patch Arknights setelah entry lama itu ditambahkan) — itu bukan berarti metodologi lama salah, cuma snapshot lama sudah stale. Belum diperbaiki, di luar scope batch ini.
- Almond skill 2 (Power Traction) level M3: wiki nulis "190%" tapi `ArknightsAssets/ArknightsGamedata` **dan** `Kengxxiao/ArknightsGameData_YoStar` sama-sama bilang 180% (`atk_scale: 1.8`) — dipakai yang dari JSON (180%), kemungkinan typo lama di wiki.
